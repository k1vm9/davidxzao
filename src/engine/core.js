...
function initGlobals(config) {
  global.GoatBot = {
    ...
    nmLocks:        new Map(),
    dmLocked:       false,
    globalLock:     false,   // when true, commands are frozen (panel toggle)
    silentMode:     false,   // when true, bot receives but doesn't respond
    allThreadData:  {},
    reLoginBot:     () => {},
  ...
  global.config.PREFIX = config.prefix || "/";
  ...
  // ── ZAO compatibility: global.client mirrors GoatBot + handleReply array ──
  global.client = global.GoatBot;
  global.client.handleReply = [];   // ZAO commands push reply watchers here
}
